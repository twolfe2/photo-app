...
app.controller('addImageCtrl', function($scope, $state, $auth, Upload) {
  if ($auth.isAuthenticated()) {
    $scope.addImage = () => {
      Upload.upload({
        url: '/api/images',
        data: {photo: $scope.photo, name: $scope.name}
      })
      .then(res => {
        // console.log(res);
        $state.go('images');
...
app.controller('imagesCtrl', function($scope, Images) {
  console.log('imagesCrtl');
